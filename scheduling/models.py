...
class BaseQuerySet(models.QuerySet, OccurrenceMixin):
    ...
    def sort_by_next(self, from_date=None):
        ...
        def sort_key(obj):
            ...
            return occ[0] if occ else make_aware(datetime.now())
    ...
    def for_period(self, from_date=None, to_date=None, exact=False):
        ...
        if to_date:
            ...
            filtered_qs = filtered_qs.filter(
                Q(wrap_q(start__lte=to_date) | wrap_q(start__isnull=True))).distinct()
...
class BaseOccurrence(BaseModel):
    ...
    def all_occurrences(self, from_date=None, to_date=None, limit=REPEAT_MAX):
        ...
        if not self.repeat:
            ...
            repeater = repeater.between(default_aware(from_date),
                                        default_aware(to_date), inc=True)
    ...
    def get_repeater(self):
        ...
        rule = rrule.rrulestr(self.repeat, dtstart=default_aware(self.start))
        ruleset.rrule(rule)
        return ruleset
